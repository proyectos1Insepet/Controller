            ...
            if(err){
                ...
                client.query(sprintf("SELECT valor FROM configuraciondispensador WHERE pk_idconfiguraciondispen=11;" ),function(err,result){
                    ...
                    if(err){
                        ...
                        //printport2.write("Producto   : "+frame_2.type_of_product_sold+'\n');     
    ...
    if((data[0]==='B') && (data[1]==='G') && (data[2]==='E')){                  //encabezado
        ...
        switch (identifier) {   //identificador
            ...
            case 'd':                                                           //DISCRIMINAR FORMA DE PAGO(#venta) 
                ...
                //    }
                //});         
                if(ocupaconsultaDATABASEFPL2!=1){//es para evitar que dos consultas se realicen exactamente al mismo tiempo  
            ...
            case 'f':                                                           //CREDITO_CANASTA
                ...
                for(x=0;x<=19;x++){//se hace para evitar que quede la variable quede guardada con algun serial
        ...
        switch (type_of_reception_2) {            
            ...
            case X_DISCRIMINATESALE:                                            //X - DISCRIMINAR FORMA DE PAGO(#venta)
                ...
                //    }
                //});  
                if(ocupaconsultaDATABASEFPL1!=1){//evita que dos consultas se realicen exactemente al mismo tiempo 
...
var bloqueo,muxbloqueoAB,muxdesbloqueoAB;

function watchful(error){
    
//esta condicion permite atender las solicitudes del nsx o php despues de terminar de configurar al mux     
            ...
            pg.connect(conString, function(err, client, done){
                if(err){
                    ...
                    client.query("SELECT nsxonline,bloqueocorte FROM estado ;", function(err,result){
                        ...
                        if(err){
                            ...
                            switch (bloquearsurtidor) {
                                case 1://si solicitan bloquear para corte
                                    //("     Bloqueo surtidorGENESYS" );  
                                    //enviar la trama de bloqueo surtidor
                                    bloqueo='1';
                                break;
                                case 0:// si solicitan desbloquear para corte
                                    //("     Desbloqueo surtidorGENESYS" ); 
                                    //enviar la trama de desbloqueo surtidor  
                                    bloqueo='0';                                
                                    sendMux(unlock_mux);   
                                break;
                                case 2:// estado normal y pasivo para el mux surtidor
                    ...
                    switch (bloqueo) {
                        ...
                        break;
                        case '0'://condiciones para desbloqueo del surtidor
                            if(frame_1.state==0x38 && frame_2.state==0x38){
                                //("Desbloqueo LadoA --  Desbloqueo LadoB" );                  
                            	bloqueo=0;
                            	muxdesbloqueoAB=12;//bloquea ambas posiciones
                            	sendMux(blocking_mux);
                            }
                            if(frame_1.state==0x38 && frame_2.state!=0x38){
                                //("Desbloqueo LadoA --  ocupado LadoB" );                  
                            	muxdesbloqueoAB=1;//bloquea posicion1
                            	sendMux(blocking_mux);       	
                            }
                            if(frame_1.state!=0x38 && frame_2.state==0x38){
                                //("ocupado LadoA --  Desbloqueo LadoB" );                   
                            	muxdesbloqueoAB=2;//bloquea posicion2
                            	sendMux(blocking_mux);      	
                            }   
                        break;                        
                        default:
                    ...
                    if((frame_1.state==0x16 && frame_2.state==0x16) || (frame_1.state==0x38 && frame_2.state==0x38) ){
                        // Proceso para pedir totales siempre que halla una fin ventaq en cualquiera de las dos posiciones del surtidor
...
var initialSettings_table=1,authorizedPpu_table=2,turn_table=3,button_Names_table=4,turnopenclose_table=5,not_readtable=6,
    ...
    calibration_tableL1=18,calibration_tableL2=19,Cbasket_tableL1=20,Cbasket_tableL2=21,sellerData=22;//configuraciones iniciales leer , Ppu autorizados,botones,canasta,FORMADEPAGO
...
var initialSettings_mux=1,authorizedPpu_mux=2,turnMux_Nsx=3,totalRequest_mux=4,initialbuttons_mux=5,turnopenclose_mux=6,
    ...
    acceptedPayment_mux=12,accepetFP_mux=13,accepetcalibration_mux=14,Cbasket_mux=15,blocking_mux=16,unlock_mux=17,datosvendedor_mux=18; //enviar trama al mux 
                    ...
                    if(creditEnablesReadingL1!=1){                
                        client.query("SELECT lecturacupocredito FROM preset where id_pos=1;", function(err,result){
                            ...
                            if(err){
                                ...
                                ////("*******PHP habilita lectura de credito L1*******");                        
                    ...
                    case creditbasket_tableL2:
                        if(creditEnablesReadingL2!=1){                
                            ...
                            if(err){
                                ...
                                ////("*******PHP habilita lectura de credito L2*******");                        
            ...
            if(err){
                ...
                switch (Tablenumberconsign) {
                    ...
                    case consign_tableL2://tabla de consignaciones
                        ...
                        ////("*******Habilita BGB2 que se puede leer tabla consignaciones L2*******");
                        client.query("SELECT confirmacion FROM consignaciones;", function(err,result){
                            done();
                            if(err){
...
//variables forma de pagodiscriminado, turno usuario id del vendedor
...
var ocupaconsultaDATABASEFPL1,ocupaconsultaDATABASEFPL2;//variables para evitar consultas exactamente en el mismo tiempo
            ...
            if(err){
                ...
                switch (TableNumber) {
                    case initialSettings_table:
                        ...
                        //It's state prints in the sideA and sideB
                    ...
                    break;
                    case sellerData://datos del vendedor
                        //leer tabla de datos del vendedor
...
var sendMux =function(numeroEnvio){
    ...
    switch (numeroEnvio){
        ...
        case turnnsxbbbmux_mux://se envia si se requiere abrir o cerrar turno desde el bbb por proceso turno
            ...
            TableNumber=sellerData;//ANTES finalizaba el proceso aqui pero se ingreso la solicitud de enviar datos del vendedor al mux            
            //TableNumber=not_readtable; 
            numeroEnvio='0';  
        break;   
        case datosvendedor_mux:
            ...
            var resultado=20-vendedorserial.length; //apartir de esta linea hasta las 6 subsiguientes se adicionan los espacios del mensaje
            var adding_spaces=" ";
            for(x=1;x<resultado;x++){
                adding_spaces=adding_spaces+" ";
            }
            vendedorserial=vendedorserial+adding_spaces; 
            var resultado=4-vendedorcontrasena.length; //apartir de esta linea hasta las 6 subsiguientes se adicionan los espacios del mensaje
            var adding_spaces=" ";
            for(x=1;x<resultado;x++){
                adding_spaces=adding_spaces+" ";
            }
            ...
            //("primer variable"+vendedorserial);
            //("segunda variable"+vendedorcontrasena);
            //asegurar que vendedor serial sea de 20 datos
            //asegurar que vendedor contraseña sea de 4 datos
            if(L1_request>0 && L1_request!=undefined){
        ...
        case responseto_requestpayment_mux:
            ...
            if(L1_request>0 && L1_request!=undefined){
                ...
                //salevalueFP=String(salevalueFP);                 
...
setInterval(watchful,1000);           //Revisa el estado de las banderas
